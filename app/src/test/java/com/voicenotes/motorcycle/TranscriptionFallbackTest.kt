...
class TranscriptionFallbackTest {
    ...
    fun testEmptyTranscriptionResultsInFallbackStatus() {
        println("TEST: Empty transcription should result in FALLBACK status")
        ...
        assertEquals("Updated recording should have empty v2sResult", 
            "", updatedRecording.v2sResult)
        
        println("  ✓ TEST PASSED: Empty transcription correctly results in FALLBACK status")
    }

    @Test
    fun testBlankTranscriptionResultsInFallbackStatus() {
        println("TEST: Blank transcription (spaces/tabs/newlines) should result in FALLBACK status")
        ...
        println("  ✓ TEST PASSED: All blank variations correctly result in FALLBACK status")
    }
    ...
    fun testFallbackRecordingValidation() {
        println("TEST: Recording with FALLBACK status should be valid")
        ...
        // Verify consistency
        assertTrue("FALLBACK status should have empty result", 
            recording.v2sResult.isNullOrBlank())
        ...
        println("  ✓ TEST PASSED: FALLBACK recording is valid and consistent")
    }

    @Test
    fun testStatusTransitionToFallback() {
        println("TEST: Status transition from PROCESSING to FALLBACK")
        ...
        println("  After empty transcription: v2sStatus = ${fallback.v2sStatus}, v2sFallback = ${fallback.v2sFallback}")
        ...
        assertEquals("v2sResult should be empty", "", fallback.v2sResult)
        
        println("  ✓ TEST PASSED: Status correctly transitions from PROCESSING to FALLBACK")
    }
    ...
    fun testEdgeCasesSingleSpaceTranscription() {
        println("TEST: Edge case - single space should be treated as blank")
        ...
        println("  ✓ TEST PASSED: Single space correctly treated as blank")
    }
    ...
    fun testEmptyChunksResultInFallback() {
        println("TEST: Empty result chunks should result in FALLBACK")
        ...
        // Verify this results in FALLBACK status
        val expectedStatus = if (joinedText.isBlank()) V2SStatus.FALLBACK else V2SStatus.COMPLETED
        assertEquals("Empty chunks should result in FALLBACK status", 
            V2SStatus.FALLBACK, expectedStatus)
        
        println("  ✓ TEST PASSED: Empty chunks correctly result in FALLBACK")
    }
